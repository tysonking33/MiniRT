...
struct s_vector3 *ray_color(struct s_ray *r, struct s_scene *scene)
{
    ...
    {
        struct s_vector3 *N = normalize(vector_subtract(ray_at(r, t), &(struct s_vector3){0, 0, -1}));
        struct s_vector3 *result = scalar_multiply(vector_add(N, &(struct s_vector3){1.0, 1.0, 1.0}), 0.5);
        free(N); // Free memory allocated for N
        return result;
    }
    ...
    struct s_vector3 *result = vector_add(term1, term2);

    // Free memory for unit_direction
    // Free memory for unit_direction
    free(unit_direction);
...
void draw_scene(struct s_scene *scene, struct s_data *img, void *mlx, void *win)
{
    ...
    int final_j = 0;
    int final_i = 0;
    //printf("image_height: %f, image_width: %f\n", image_height, image_width);
    ...
    for (int j = 0; j < image_height; ++j)
    {
        ...
        {
            // Allocate memory for pixel_center
            ...
            // Free allocated memory for pixel_center
            free(pixel_center);

            plot_pixel(mlx, win, img, i, j, color_int);
            final_i = i;
            final_j = j;
            /*// Free allocated memory for pixel_center, ray_direction, and pixel_color
            free(pixel_center);
            free(ray_direction);
            free(pixel_color);*/
        }
...
int begin_drawing(struct s_scene *myScene, struct s_data *data)
{
    ...
    {
        plot_pixel(data->mlx_and_win->mlx, data->mlx_and_win->win, data, 5, i, 0xFFFFFF00);
        plot_pixel(data->mlx_and_win->mlx, data->mlx_and_win->win, data, 10, i, 0xFFFFFF00);
    }*/
